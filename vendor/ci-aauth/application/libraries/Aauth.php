...
class Aauth {
	...
	public function login($identifier, $pass, $remember = FALSE, $totp_code = NULL) {
		...
		// not matches
		if( !$query->num_rows() OR !$this->verify_password($password, $row->pass) ){
			$this->error($this->CI->lang->line('aauth_error_login_failed_all'));
			return FALSE;
		}
		// Email/name and pass matches
		// create session
		$data = array(
			'id' => $row->id,
	...
	public function control( $perm_par = FALSE ){
		...
		if($this->CI->session->userdata('totp_required')){
			...
			$this->CI->session->return_uri = uri_string();
		...
		if ( ! $this->is_allowed($perm_id) ){
			...
			elseif(($noaccess = $this->config_vars['no_access']) && is_callable($noaccess)){
				die($noaccess());
 			}
			else {
	...
	public function update_login_attempts() {
		...
		if($query->num_rows() == 0){
			...
			$data = array();
			$data['timestamp'] = date("Y-m-d H:i:s");
			$data['count'] = $row->login_attempts + 1;
			$this->aauth_db->where('id', $row->id);
			$this->aauth_db->update($this->config_vars['login_attempts'], $data);

			return $data['count'] <= $this->config_vars['max_login_attempt'];
		}
	...
	public function get_login_attempts() {
		...
		$query = $this->aauth_db->get( $this->config_vars['login_attempts'] );
		return $query->num_rows() ? $query->row()->count : 0;
	}
	 ...
	 */
	public function create_group($group_name, $definition = '', $main_perm=null) {
